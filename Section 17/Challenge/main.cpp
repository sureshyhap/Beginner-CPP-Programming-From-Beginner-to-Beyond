...
std::unique_ptr<std::vector<std::shared_ptr<Test>>> make() {
  std::unique_ptr<std::vector<std::shared_ptr<Test>>> pv = std::make_unique<std::vector<std::shared_ptr<Test>>>();
  return std::move(pv);
}
...
void fill(std::vector<std::shared_ptr<Test>>& vec, int num) {
  for (int i = 1; i <= num; ++i) {
    ...
    std::cin >> data;
    std::shared_ptr<Test> pt = std::make_shared<Test>(data);
